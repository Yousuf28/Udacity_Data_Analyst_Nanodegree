---
title: "Investigating on Medical Appointment No Show Dataset"
author: "Yousuf Ali"
date: "10/4/2018"
output: 
...
```{r}
BarPlotStacked <- function(x, na.rm = TRUE, ...) {
  nm <- names(x)
  for (i in seq_along(nm)) {
print(ggplot(x,aes_string(x = nm[i], fill = tail(nm, n=1))) + geom_bar(alpha = .5)) }
}
```

```{r}
BarPlotCount <- function(x, na.rm = TRUE, ...) {
  nm <- names(x)
  for (i in seq_along(nm)) {
print(ggplot(x,aes_string(x = nm[i])) + geom_bar(alpha = .5, fill = 'salmon')) }
}
```

```{r}
BarPlotProp <- function(x, na.rm = TRUE, ...) {
  nm <- names(x)
  for (i in seq_along(nm)) {
print(ggplot(x,aes_string(x = nm[i], fill = tail(nm, n=1))) + geom_bar(alpha = .5, position = position_fill())) }
}
```

```{r}
df_plot_1 <- select(df, scholarship, 
               hypertension,
               diabetes, 
               alcoholism, 
               handicap, 
...
```{r}
BarPlotCount(df_plot_1)
```

```{r}
BarPlotStacked(df_plot_1)
```

```{r}
BarPlotProp(df_plot_1)
```
...
[double click and open new tab here](https://www.kaggle.com/yousuf28/medical-appointment-no-show-in-python/notebook)


If you want to see same plot in **Tableau** <br> and want to see some **SAS** code 
go to my github page [double click and open new tab here:github](https://github.com/Yousuf28/udacity_data_analyst_nano_degree/blob/master/README.md)